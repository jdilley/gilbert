...
/**
 * Select — single-choice menu primitive.
 *
 * Hairline-bordered trigger, hairline-bordered popover, signal-colored
 * indicator on the active item. Density matches Button — h-7 default,
 * h-6 ``sm``. The trigger gets a focus outline rather than a heavy
 * ring; matches the system's outline-over-fill rule.
 *
 * Group labels render in the eyebrow style (uppercase mono, 11px) so
 * a select that groups its options reads as a small section.
 */
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        "rounded-md border border-input bg-transparent pl-2.5 pr-2",
        "text-sm leading-none transition-[border-color,background-color] duration-(--duration-fast) ease-(--ease-out)",
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn(
            ...
            // Popovers float off-canvas so a SMALL shadow is allowed
            // to encode elevation. Keep it tight — no 24px blurs.
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        ...
        "py-1 pr-8 pl-2 text-sm leading-snug",
        "outline-hidden select-none",
        // Hover / keyboard focus — subtle bg + foreground tint.
        "focus:bg-foreground/8 focus:text-foreground",
        // Selected — signal-tinted left rail + bg.
        "data-[selected=true]:bg-(--signal)/10 data-[selected=true]:text-foreground",
    ...
    >
      ...
      <SelectPrimitive.ItemIndicator
        render={
          <span className="pointer-events-none absolute right-2 flex size-3.5 items-center justify-center text-(--signal)" />
        }
      >
